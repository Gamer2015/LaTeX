                                ...
                                request.onsuccess = function(event) {
                                    ...
                                    for(let package of serverData.packages) {
                                        ...
                                        packageStore.put({
                                            ...
                                            key: decrypt(account.dataKey, package.key),
                                            data: decrypt(account.dataKey, package.data),
                                        });
                                    }
            ...
            if(accounts.length != 0) {
                websocket.send("save", {
                    accounts: accounts.map(function(account) { return {
                        ...
                        let account = accountMap[package.accountId];
                        return {
                            key: encrypt(account.dataKey, package.key),
                            data: encrypt(account.dataKey, package.data),
                            accountId: package.accountId
                        }
                    })
                    , timestamp: Date.now()
