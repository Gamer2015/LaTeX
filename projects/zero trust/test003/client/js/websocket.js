...
function populateNewPackageIds(packages, callback) {
    ...
    // get temporary package ids, reusing deleted package ids, that are new in out local database
    let tmpIds = new Set();
    ...
    window.indexedDB.open('database').onsuccess = function(event) {
        ...
        let packageStore = transaction.objectStore('packages');
        let deletedIndex = packageStore.index('deleted');
        deletedIndex.count(IDBKeyRange.only(indexedDbTrue)).onsuccess = function(event) {
            let deletedCount = event.target.result;
            if(deletedCount >= unpopulatedCount) {
                // reusing deleted packages is sufficient
                // get random selection of a few of them
                let selectedIndices = new Set();
                while(selectedIndices.size < unpopulatedCount) {
                    let missing = deletedCount - selectedIndices.size;
                    generateRandom(missing).reduce(function(selected, generated) {
                        selected.add(Math.abs(generated) % deletedCount);
                        return selected;
                    }, selectedIndices);
                }
                let advances = Array.from(selectedIndices).sort();
                for(let i = advances.length; i > 0; --i) {
                    advances[i] -= advances[i-1];
                }
                let current = -1;
                deletedIndex.openCursor(IDBKeyRange.only(indexedDbTrue)).onsuccess = function(event) {
                    if(advances.length == 0) {
                        return;
                    }

                    ++current;
                    let advance = advances[current];
                    if(current == 0 && advance > 0) {
                        // the first index is not 0, so jump to the desired first index
                        cursor.advance(advance);
                    }
                    let cursor = event.target.result;
                    let package = cursor.value;
                    tmpIds.add(package.id);

                    if(current < advances.length) {
                        cursor.advance(advance);
                    }
                }
            }
                ...
                packageStore.openCursor().onsuccess = function(event) {
                    ...
                    if(cursor) {
                        let package = cursor.value;
                        if(package.deleted == indexedDbTrue) {
                            // use ids of all deleted records
                            tmpIds.add(package.id);
                        } 
                        // count all of them so we can gurantee global uniqueness
        ...
        transaction.oncomplete = function() {
            tmpIds = tmpIds.values();
            for(let package of packages) {
                let tmpId;
                if(package.id == null || package.id.length == 0) {
                    let tmpId = 0;
                    if(tmpIds.done == false) {
                        tmpId = tmpIds.next().value;
                    } else {
...
function resolveInsertedPackagesCallback(serverData, clientData) {
    ...
    window.indexedDB.open('database').onsuccess = function(event) {
        ...
        let createdPackages = successful.filter(function(package) { return package._id != null});
        console.log("createdPackages", JSON.parse(JSON.stringify(createdPackages)));
        ...
        let unsucessfulNewPackages = failed.filter(function(package) { return package._id != null});
        for(let package of unsucessfulNewPackages) {
            let key = [package.accountId, package._id];
            packageStore.delete(key);
            delete package._id;
        }
