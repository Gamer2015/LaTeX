...
def unpackPackages(packages):
    tmps = unpack(packages, ["username", "id", "data", "key"])
    return hex2bytes(tmps, ["id"])
...
def packPackages(packages, excludes=None):
    ...
    tmps = unpack(packages, [
        prop for prop in ["username", "id", "data", "key"] 
...
def packLogins(logins, excludes=None):
    if excludes == None:
        excludes = []
    excludes = forceSet(excludes)

    tmps = unpack(logins, [
        prop for prop in ["username", "id"] 
        if prop not in excludes
    ])
    return bytes2hex(tmps, [
        prop for prop in ["id"] if prop not in excludes
