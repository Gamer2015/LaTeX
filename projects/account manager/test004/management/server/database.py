...
def generateLoginId():
    ...
    while id[0] == 0:
        id =  secrets.token_bytes(idLength)
    return id
def createLogins(dbFileName, accounts, amount):
    ...
    for account in successAccounts:
        loginCounter[account["id"]] = 0

    total = amount * len(successAccounts)
    logins = []
    timestamp = datetime.datetime.now()
    while len(logins) < total:
        cycleLogins = []
        for account in successAccounts:
            for i in range(amount-loginCounter[account["id"]]):
                cycleLogins.append({
                    "accountId": account["id"],
                    "username": account["username"]
                })

        insertEntities(db, c, 'logins', cycleLogins, generateLoginId, 'id')
        createdLogins = ([
            login["entity"] for login in cycleLogins if login["status"] == "success"
        ])
        for login in createdLogins:
            loginCounter[login["accountId"]] = loginCounter[login["accountId"]] + 1

        logins.extend(createdLogins)

    return logins, failedAccounts
...
def insertEntities(db, cursor, table, entities, keyGenerator, keyPath):
    for entity in entities:
        ...
        try:
            ...
            makeStatusEntity(entity, "failed", exception)
...
def createLoginDatabase(database, cursor):
    cursor.execute("""
        CREATE TABLE logins (
        id blob PRIMARY KEY
        , accountId blob NOT NULL
    )""")
    cursor.execute('CREATE INDEX accountId ON logins(accountId)')
    database.commit()

def createActiveLoginDatabase(database, cursor):
    cursor.execute("""
        CREATE TABLE activeLogins (
        loginId blob NOT NULL
        , deviceId blob NOT NULL
        , expiration integer 
        , PRIMARY KEY(loginId, deviceId)
    )""")
    cursor.execute('CREATE INDEX deviceId ON activeLogins(deviceId)')
    database.commit()
