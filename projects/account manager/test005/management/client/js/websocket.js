...
function createIdentitiesCallback(serverData, clientData) {
    if(serverData.identities.length != 0) {
        info("new identities", serverData.identities);
        let usersChanged = serverData.identities.reduce(function(usernames, login) {
        ...
        let packages = [];
        for(let identity of serverData.identities) {
            let account = storedAccounts[login.username];
            // create packages from identities
            packages.push({
    ...
    if(serverData.failed.length != 0) {
        error("getting new identities failed", serverData.failed);
...
function createIdentities(
    ...
    , callback=createIdentitiesCallback
) {
    ...
    websocket.send("create identities", {
