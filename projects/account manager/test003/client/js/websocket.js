...
function refreshPackagesCallback(serverData, clientData) {
    if(serverData.failed.length == 0 && serverData.packages.length == 0) {
        // success, all of the new packages were retrieved
    ...
    if(serverData.packages.length != 0) {
        ...
        let successUsernames = serverData.packages.reduce(function(usernames, package) {
            usernames.add(package.username);
            return usernames;
        }, new Set());

        let storedAccounts = getAccounts(successUsernames);
        for(let username of successUsernames) {
            if(username in storedAccounts) {
        ...
        for(let package of serverData.packages) {
            ...
            account.packages[package.packageId] = package;
        }
...
let relevantPackageFields = ["username", "packageId", "data", "key"];
