...
function initializeDatabase(callback) {
	...
	let upgrades = [{
		...
		, upgrade: function(database) {
	    	let accountStore = database.createObjectStore('accounts', { keyPath: 'name' }); 
		...
		date: '2020-12-22'
		, author: 'stefan kreiner'
		, summary: 'create account package database'
		, database: 'database'
		, upgrade: function(database) {
			database.createObjectStore('accountPackages', { keyPath: ['accountName', 'packageId'] });  
		}, success: function(database, callback) {
			if(callback) {
				callback();
			}
