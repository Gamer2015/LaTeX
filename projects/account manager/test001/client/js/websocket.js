...
function requestLoginsCallback(serverData, clientData) {
    ...
    info("Accounts created successfully, please log in.");
...
function requestLoginsFromMemory(logins, success, error) {
    websocket.send("create logins", {
        ...
        , callback: {
            ...
            , success: success
            , error: error
            , callback: requestLoginsCallback
        }
    });
}
// server-side database operation integration
function requestLogins(success, error, number=1) {
    let accounts = generateSecrets(number).map(function(secret) { return {
            secret: secret,
    }});
    // sjcl.decrypt("password", "encrypted-data")
    requestAccountsFromMemory(logins, success, error);
}
