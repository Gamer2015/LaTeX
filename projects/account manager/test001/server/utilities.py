def forceIterable(entities):
    ...
    try:
        # do not consider string an iterable
...
def forceSet(entities):
    ...
    try:
        # do not consider a string as list of characters
        if(isinstance(entities, str)):
            raise Exception
        # do not consider object as set of keys
...
def packLogins(accounts, excludes=None):
    if excludes == None:
        excludes = []
    excludes = forceSet(excludes)

    tmps = unpack(accounts, [
        prop for prop in ["accountId"] if prop not in excludes
    ])
    return bytes2hex(tmps, [
        prop for prop in ["accountId"] if prop not in excludes
    ])
...
def unpackPackages(packages):
    tmps = unpack(packages, ["accountId", "packageId", "data"])
    return hex2bytes(tmps, ["accountId", "packageId"])
...
def packPackages(packages, excludes=None):
    ...
    tmps = unpack(packages, [
        prop for prop in ["accountId", "packageId", "data"] if prop not in excludes
    ])
    return bytes2hex(tmps, [
        prop for prop in ["accountId", "packageId"] if prop not in excludes
    ])
