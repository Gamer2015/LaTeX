let database = (function() {
	...
	return {
		...
		prepareDatabases: function(callback) {
			...
			this.queueUpgrade(
				'2020-12-12 (1): create personas store and save accounts there'
				, 'messenger'
				, function metadata() {
					if(this.messenger == null) {
						this.messenger = {};
					}
					this.messenger.personas = new ObjectStore(
						this.messenger
						, 'personas'
						, 'id'
					); 
					...
					let db = event.target.result;
			    	db.createObjectStore(this.messenger.personas.name(), {
			    		keyPath: this.messenger.personas.keyField() 
			    	});
				}.bind(this), function records(database, callback) {
					let transaction = database.transaction([
						this.messenger.personas.name()
						, this.messenger.accounts.name()
					], 'readwrite');

					let accountsStore = transaction.objectStore(this.messenger.accounts.name());
					let personasStore = transaction.objectStore(this.messenger.personas.name());
					
					let cursor = accountsStore.openCursor();
					cursor.onerror = function(event) {
						error('Error', event.target.error.name);
					}
					cursor.onsuccess = function(event) {
						let cursor = event.target.result;
						if(cursor) {
							personasStore.put(cursor.value);
							cursor.continue();
						} else {
							if(callback) {
								callback();
			...
			this.queueUpgrade(
				'2020-12-12 (2): delete accounts store 1'
				, 'messenger'
				, null
				, function stores(event) {
					let db = event.target.result;
					db.deleteObjectStore(this.messenger.accounts.name());
				}.bind(this));

			this.queueUpgrade(
				'2020-12-12 (3): delete accounts store 2'
				, 'messenger'
				, function metadata(callback) {
					delete this.messenger["accounts"];
				}.bind(this));
