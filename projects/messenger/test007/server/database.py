...
def authenticate(dbFileName, channels):
    ...
    c.execute('SELECT id, passwordHash FROM ' + passwordsTableName + ' WHERE id in ({seq})'.format(
        seq=','.join(['?']*len(ids))
    ), ids) 
...
def init(dbFileName):
    ...
    try: # initialize database
        c.execute('CREATE TABLE ' + versionsTableName + ' (version int, date text)')
        n = datetime.datetime.now() 
        c.execute('INSERT INTO ' + versionsTableName + ' VALUES (0, ?)', (n,))   
        # save changes
        db.commit() 
    except Error as e:
